...
import com.yanzhenjie.album.util.SystemUtil;
...
public class AlbumActivity extends BaseActivity implements
     ...
     * 4 对花调用相册，
     *
     * 5 MainActivity upload掉用相册,
     * 6 UploadActivity upload调用相册,
     *
     * 7 pad版多图修复上传,MainActivity掉用相册
     * 8 pad版多图修复上传 PicturesUploadActivity 掉用相册
     *
     */
...
//                .start();
    ...
    private Action<CameraCallBack> mCameraAction = new Action<CameraCallBack>() {
        ...
        public void onAction(@NonNull CameraCallBack result) {
            ...
            task.execute(result.getmCameraFilePath(),result.getmUriPath());
        }
    };
