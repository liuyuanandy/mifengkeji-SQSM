...
public class PathConversion {
    ...
    @WorkerThread
    @NonNull
    public AlbumFile convert(String filePath,String uriContentPath) {
        File file = new File(filePath);
        AlbumFile albumFile = new AlbumFile();
        albumFile.setPath(filePath);
        albumFile.setUriContentPath(uriContentPath);
        ...
        if (mediaType == AlbumFile.TYPE_VIDEO) {
            ...
            try {
                if(Build.VERSION.SDK_INT > Build.VERSION_CODES.P){
                    albumFile.setDuration(getVideoTime(context,uriContentPath));
                }else{
    ...
    private long getVideoTime(Context context,String uriContentPath) {
        long durtion=0;
        String[] VIDEOS = {
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                String QPath = MediaStore.Video.Media
                        .EXTERNAL_CONTENT_URI
                        .buildUpon()
                        .appendPath(String.valueOf(id)).build().toString();
                if(uriContentPath.equals(QPath)){
                    durtion = cursor.getLong(0);
                    return durtion;
                }
            }
            cursor.close();
