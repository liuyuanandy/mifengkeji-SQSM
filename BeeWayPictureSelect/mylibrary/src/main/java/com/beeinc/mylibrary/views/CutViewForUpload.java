...
import com.beeinc.mylibrary.activity.CutPictureActivity;
import com.beeinc.mylibrary.activity.DuiHuaActivity;
import com.beeinc.mylibrary.activity.MirroPictureActivity;
...
public class CutViewForUpload extends View {
    ...
    private int width_use;//unity 调用时设置的宽
    private int height_use;//unity 调用时设置的高
    private int statusBarHeight=-1;//状态栏高度
    private CutViewListener listener;
    public int getWidth_use() {
        return width_use;
    }

    public int getHeight_use() {
        return height_use;
    }

    public CutViewListener getListener() {
        return listener;
    }

    public void setListener(CutViewListener listener) {
        this.listener = listener;
    }

    public CutViewForUpload(Context context) {
        super(context);
    }


    public CutViewForUpload(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public CutViewForUpload(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void onDraw(Canvas canvas){
        ...
        //绘制图片
        drawBitmap(canvas);
        if(type!=2&&!isDenyCut){//预览时不绘制线段和圆
            //绘制线段
            drawLines(canvas);
            //绘制圆
            drawCircles(canvas);
...
//        super.onTouchEvent(event);
     ...
     * @param bottomPadding
     */
    public void setBitmap(Bitmap bitmap,Bitmap preViewBitmap, int leftPadding, int topPadding, int rightPadding, int bottomPadding){
        type = 2;
        if(statusBarHeight==-1){
            statusBarHeight = StatusBarUtils.getStatusBarHeight((Activity) context);
        }
        width = ScreenUtil.SCREEN_THIS_W;
        height = ScreenUtil.SCREEN_THIS_H-statusBarHeight;
        if(this.bitmap!=null){
            this.bitmap.recycle();
            this.bitmap = null;
        }
    ...
    private void setJiaozhenAgain(){
        if(listener!=null){
            listener.setJiaozhengAgin();
        }
    }
    ...
    public void saveBitmap(String path){
        ...
        }else if(getContext() instanceof UploadActivity){
            if(type==0){//未操作
                b= bitmap;
            }else if(type==1){//操作中
                preViewClick();
    ...
    private void setWidthAndHeight(){
        ...
        if(remapWidth>remapHeight){
            width_use = DEFAULT_MAX_WITH;
            height_use = remapHeight*DEFAULT_MAX_WITH/remapWidth;
        }else{
            width_use = remapWidth*DEFAULT_MAX_WITH/remapHeight;
            height_use = DEFAULT_MAX_WITH;
        }
        if(listener!=null){
            listener.setWidthAndHeight(width_use,height_use);
        }
    }
