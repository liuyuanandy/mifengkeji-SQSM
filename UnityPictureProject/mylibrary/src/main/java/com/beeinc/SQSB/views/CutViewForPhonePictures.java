...
public class CutViewForPhonePictures extends View {
    ...
    private int type = 0;// 0 未操作 ;1 操作中 2 已点击预览 3 已矫正
    ...
    public CutViewForPhonePictures(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public CutViewForPhonePictures(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void onDraw(Canvas canvas){
        ...
        //绘制图片
        drawBitmap(canvas);
        if(type!=2&&type!=3){//预览和已矫正时不绘制线段和圆
            //绘制线段
            drawLines(canvas);
            //绘制圆
            drawCircles(canvas);
...
//      super.onTouchEvent(event);
        if(type==2||type==3){//预览中不能操错,已保存文件不能操作
            return true;
        }
    ...
    public void setBitmap(Bitmap bitmap,Bitmap previewBitmap,int type, int leftPadding, int topPadding, int rightPadding, int bottomPadding){
        this.type=type;
    ...
    private void getJiaozhenBitmap(){
        ...
        int width = getRemapedWidth();
        int height = getRemapedHeight();
    ...
    public void saveBitmap(String path){
        ...
        Mat temp = new Mat();
        Imgproc.cvtColor(mat, temp, Imgproc.COLOR_RGBA2BGRA);
...
//        }
    ...
    public interface CutViewListener{
        ...
        public void preViewClickType(int type);
    }
